...
extern const char* DB_NAME = "Energopromservice.sqlite";
extern const int DAYS_IN_MONTH = 30;
...
class Contract {
public:
	Contract(const std::string&, const std::string&, const int, const int, const double);
private:
	string name;
	...
	vector<int> order_in_overlimit_pri;
	vector<int> order_in_underlimit_pri;
	void ReorderContracts();
};
